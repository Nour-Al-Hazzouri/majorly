...
- **Bundler**: Turbopack (default for development in modern Next.js)
...
- **HTTP Client**: Native `fetch` (recommended) with TanStack Query for client caching when needed
...
#### `page.tsx` Composition-Only Rule (Hydration Safety)
**CRITICAL**: `page.tsx` files must be primarily composition.

- **Allowed in `page.tsx`**: import components, render component tags, pass props, define metadata.
- **Not allowed in `page.tsx`**: complex UI logic, heavy data shaping/mapping, DOM/browser APIs, client hooks, or embedding interactive logic directly.
- **Rule of thumb**: if a page needs non-trivial logic, move it into:
  - A **Server Component** (separate file) for data fetching/composition, and/or
  - A **Client Component** (separate file with `'use client'`) for interactivity.
...
- **Next.js Features**: Use stable Next.js caching/data-fetching patterns; avoid experimental features unless required by the Project PRC
...
- **Linting**: Use ESLint 9 with flat config (`eslint.config.js`) plus Next.js recommended rules
...
9. Leverage React 19 and Next.js 16+ features
